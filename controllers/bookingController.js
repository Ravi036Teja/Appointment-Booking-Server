...
// // This file only needs a few functions now. The payment-related logic has moved.
...
// // The `bookSlot` function is no longer needed here.
// // The `pay` endpoint in the PhonePe router creates a new booking with a "Pending" status.

// exports.getBookedSlots = async (req, res) => {
//   const { date } = req.params;
//   try {
//     // Only show "Paid" and "Pending" bookings as unavailable to others.
//     // The `Pending` status is crucial here to prevent double-bookings while a payment is in progress.
//     const bookings = await Booking.find({ date, status: { $in: ["Paid", "Pending"] } });
//     const slots = bookings.map((b) => b.timeSlot);
//     res.json(slots);
//   } catch (error) {
//     console.error("Error fetching booked slots:", error);
...
const dayjs = require("dayjs");
const Booking = require("../models/Booking");
const sendWhatsAppMessage = require("../utils/whatsappSender"); // Assuming this is for your business logic
const isSameOrBefore = require("dayjs/plugin/isSameOrBefore");
dayjs.extend(isSameOrBefore);
...
exports.getBookedSlots = async (req, res) => {
    const { date } = req.params;
    try {
        // Query for bookings that are definitively unavailable. This is the core of the slot-blocking logic.
        // It includes 'Paid' (permanently booked) and 'Pending' (temporarily locked) slots.
        const bookings = await Booking.find({ 
            date, 
            status: { $in: ["Paid", "Pending"] } 
        });
